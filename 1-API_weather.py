...
for city in city_list:
    resp = requests.get(
        f"https://nominatim.openstreetmap.org/?q={city}&format=json&countrycodes=1974&limit=1"
    ).json()
    lat = resp[0]["lat"]
    long = resp[0]["lon"]
...
def get_weather_forecasts(latitude, longitude):
    resp = requests.get(
        f"http://api.openweathermap.org/data/2.5/forecast?lat={latitude}&lon={longitude}&appid=c57023819bfd474995f3bf1b69e50c0d"
    )
...
city_weather_by_day = (
    city_weather.groupby([city_weather["city"], city_weather["dt_object"].dt.date])
    .agg(
        {
...
top_city = (
    city_weather_by_day.groupby("city", sort=False)
    .mean()
    .reset_index()
    .sort_values(["precipitation", "temperature"], ascending=[True, False])
)
